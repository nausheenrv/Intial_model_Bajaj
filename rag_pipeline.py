...
class EnhancedRAGPipeline:
    ...
    def query_rag(self, query_text: str, include_metrics: bool = False) -> Dict[str, Any]:
        """
        Enhanced RAG query with classification and context summarization
        ...
        try:
            ...
            # Retrieve relevant documents directly with the original query
            ...
            if was_summarized and include_metrics:
...
if __name__ == "_main_":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--load-pdfs":
            ...
            result = pipeline.query_rag(query, include_metrics=True)
            print(f"\n{'='*60}")
            print("FINAL RESULT")
            print(f"{'='*60}")
